...
The Vault Oracle UTxO holds config and LP state. This is the only UTxO committed to Hydra.
...
- `app_oracle`: PolicyId
- `vault_script_hash`: ByteArray (links to the Vault holding LP funds)
...
1. CreateVault - `CreateVault { initial_deposit, initial_lp, prices_message, signatures, initial_lp_proof }`
   ...
   - Initial state: `total_lp = initial_lp`, `operator_lp = initial_lp`, `vault_cost` from price calculation
...
- **Funds separation**: LP funds stay in Vault on L1; only Oracle state enters Hydra
